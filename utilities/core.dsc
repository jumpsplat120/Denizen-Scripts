has_permission:
    ...
    debug: false
    script:
        - define perms <[player].flag[simple_permissions].as_map>
        - if <proc[deep_get].context[<[perms]>|<[permission]>]>:
            - determine true
        - define chain_ls <list>
        - foreach <[permission].split[.]>:
            - define chain <tern[<[loop_index].is[==].to[1]>].pass[<[value]>].fail[<[chain]>.<[value]>]>
            - define chain_ls <[chain_ls].include[<[chain]>.*]>
        - foreach <[chain_ls].remove[last].reverse>:
            - if <proc[deep_get].context[<[perms]>|<[value]>]>:
                - determine true
        - determine false
...
n:
    ...
    debug: false
    script:
        - determine <proc[clamp].context[<[val]>|0|1]>
